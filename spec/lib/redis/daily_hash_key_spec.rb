...
RSpec.describe Redis::DailyHashKey do
  let(:mock_class) do
    Class.new do
      include Redis::Objects

      daily_hash_key :browsing_history
  ...
  context 'with global: true' do
    let(:mock_class) do
      Class.new do
        ...
        daily_hash_key :browsing_history, global: true
      end
    end
    ...
    it 'supports class-level increment/decrement of global counters' do
      expect(homepage.redis.hgetall('homepage::browsing_history:2021-04-01'))
        .to eq({ 'item1' => '1.5', 'item2' => '2' })
      expect(homepage.redis.hgetall('homepage::browsing_history:2021-04-02'))
        .to eq({ 'item1' => '3', 'item2' => 'a', 'item3' => '5' })
      expect(homepage.redis.hgetall('homepage::browsing_history:2021-04-03'))
  ...
  describe 'keys' do
    it 'appends new counters automatically with the current year' do
      expect(homepage.redis.hgetall('homepage:1:browsing_history:2021-04-01'))
        .to eq({ 'item1' => '1.5', 'item2' => '2' })
      expect(homepage.redis.hgetall('homepage:1:browsing_history:2021-04-02'))
        .to eq({ 'item1' => '3', 'item2' => 'a', 'item3' => '5' })
      expect(homepage.redis.hgetall('homepage:1:browsing_history:2021-04-03'))
        .to eq({ 'item2' => '1', 'item3' => '2', 'item4' => '1' })
    end
  end
